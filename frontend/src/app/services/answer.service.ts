...
export class AnswerService {
  ...
  constructor(private qservice: QuestionnaireService) {
    qservice.loaded.subscribe((q) => {
      ...
      this.selectedStorage = q.questions.map((q) => new Array(q.choices.length).fill(false));
      this.update();
    });
  }
  ...
  goto(q: number) {
    if (q >= 0 && q < this.questionnaire.questions.length) {
      this.selectedStorage[this.currentQuestion] = this.selected;
  ...
  updateSelection(event: MatSelectionList) {
    ...
    this.selectedStorage[this.currentQuestion] = this.selected.map((_, i) => {
  ...
  update() {
    ...
    if (!this.empty) {
      ...
      this.selected = this.selectedStorage[this.currentQuestion];
